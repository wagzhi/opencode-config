...
每次进入任务执行前，先在写入计划文件和修改代码之前检查 Git 状态：

1. 若工作区存在未提交或未跟踪修改，暂停执行，列出变更文件，并询问用户是否继续。
2. 若工作区干净且当前在 `dev`、`master` 或 `release/*`、`release-*` 分支上，通过 `@git-agent` 创建并切换到 `feat/<brief_name>` 分支。
3. 若当前已在本任务分支上，继续执行；若当前是其他任务分支，询问用户是基于当前分支创建新分支，还是切回 `dev` 后创建新分支。
4. 分支名使用 `feat/` 前缀，任务英文简述使用小写和下划线连接，例如 `feat/ticket_order_pay`。
