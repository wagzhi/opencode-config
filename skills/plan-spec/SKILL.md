...
本技能可能安装在不同位置。引用附属文档时，应始终基于当前 `SKILL.md` 所在的 skill 目录解析相对路径。
...
- 处理 gitee issue 读取、评论、创建任务 issue、更新状态时，依赖已配置且可用的 Gitee MCP。
- 执行 gitee 相关操作前，应优先使用 Gitee MCP 工具；如果当前环境未配置、无权限或调用失败，应停止对应的 gitee 同步步骤，并向用户说明缺失的能力和需要补齐的信息。
- 不要猜测仓库 owner、repo、issue number 或 issue 类型；这些信息应来自用户输入、当前上下文或 Gitee MCP 查询结果。
- 若无法通过 Gitee MCP 确认 issue 类型为“需求”，不得自动进入需求拆解创建任务流程。
- 判定“remote 指向 gitee”时，应基于 git remote URL（如 `origin`）包含 `gitee.com` 或等效 gitee 域名，不得凭经验假定。
- 在自动创建需求/任务 issue 前，应先通过 Gitee MCP 进行查重；若已存在可复用 issue，则优先复用并建立关联，避免重复创建。

## Workflow

### 未指定 Issue 的自动创建与闭环

当项目 git 的远程仓库（`remote`）指向 gitee，且当前任务未指定对应的 gitee issue 时：

1. 使用 Gitee MCP 在对应仓库先创建“需求” issue，再基于该需求拆解并创建一个或多个“任务” issue。
...
5. 当该需求下任务已完成时，将对应需求 issue 状态更新为 `closed`。
...
1. 通过 gitee MCP 读取需求 issue 的标题、正文、标签、评论和当前状态。
2. 分析需求目标、用户价值、范围边界、关键约束、验收标准和主要风险。
3. 将需求拆解为一个或多个可独立执行、可验证的任务。
4. 通过 gitee MCP 为每个任务创建对应的任务 issue；任务正文应包含来源需求 issue、任务目标、实现范围、验收标准、测试建议和依赖关系。
...
- 详细规范见 `references/agent_spec.md`。
